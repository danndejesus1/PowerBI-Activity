"""
DAX Query Agent Setup
Handles LLM agent initialization, DAX documentation, and query tools
"""
import os
import json
import requests
...
# Simple cache without Streamlit (to avoid threading issues)
...
def load_dax_documentation():
    ...
    try:
        with open('dax_documentation.txt', 'r', encoding='utf-8') as f:
...
def get_relevant_dax_docs(query: str) -> str:
    ...
    # Always include critical rules and common patterns
    relevant.extend([s for s in sections if 'Critical Rules' in s or 'Common Patterns' in s])
...
# Global system prompt that will be used by the agent
SYSTEM_PROMPT = """You are a DAX query expert for Power BI data analysis.
...
def create_dax_agent():
    ...
    tools = [dax_syntax_tool, schema_tool, execute_dax_tool]
    
    # Create agent - pass model and tools only
